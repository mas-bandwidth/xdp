/*
    UDP crypto XDP program (Userspace)
...
int bpf_init( struct bpf_t * bpf, const char * interface_name )
{
    ...
    // load the crypto_xdp program and attach it to the network interface

    printf( "loading crypto_xdp...\n" );

    bpf->program = xdp_program__open_file( "crypto_xdp.o", "crypto_xdp", NULL );
    if ( libxdp_get_error( bpf->program ) ) 
    {
        printf( "\nerror: could not load crypto_xdp program\n\n");
        return 1;
    }

    printf( "crypto_xdp loaded successfully.\n" );

    printf( "attaching crypto_xdp to network interface\n" );
    ...
    else
    {
        ...
        {
            printf( "\nerror: failed to attach crypto_xdp program to interface\n\n" );
            return 1;
        }
    }
