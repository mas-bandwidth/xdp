/*
    UDP drop XDP program (Userspace)
...
int bpf_init( struct bpf_t * bpf, const char * interface_name )
{
    ...
    // load the drop_xdp program and attach it to the network interface

    printf( "loading drop_xdp...\n" );

    bpf->program = xdp_program__open_file( "drop_xdp.o", "drop_xdp", NULL );
    if ( libxdp_get_error( bpf->program ) ) 
    {
        printf( "\nerror: could not load drop_xdp program\n\n");
        return 1;
    }

    printf( "drop_xdp loaded successfully.\n" );

    printf( "attaching drop_xdp to network interface\n" );
    ...
    else
    {
        ...
        {
            printf( "\nerror: failed to attach drop_xdp program to interface\n\n" );
            return 1;
        }
    }
